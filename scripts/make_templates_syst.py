...
masses_8TeV_1 =  [115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 125, 126, 127, 128, 129, 130, 135, 140, 145, 150, 160, 170, 180, 190, 200]
masses_8TeV_2 =  [110, 115, 120, 125, 126, 130, 140, 150, 160, 180, 200]
masses_8TeV_3 =  [125,126]
masses_8TeV_4 =  [115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 127, 128, 129, 130, 135, 140, 145, 150, 160, 170, 180, 190, 200]

masses_7TeV_ggH =  [115,120,125,130,140,150,160,170, 180]
...
out_file = TFile(dirout + "hzz_templates_%4.4d_%8.8d.root" % (JOBNUM,SEED),
		                  "recreate")
...
for ichannel, channel in enumerate(channels):
    ...
    print name  
    thisfile = '{0}/output_ntuple_ZX_{2}_{1}_bnn.root'.format(dirinzx, channel, era)
#    sumweight = plot_util.getsumweight_ntuple2([thisfile])
...
for ichannel, channel in enumerate(channels):
    ...
    for thismass in masses_8TeV_1:
        ...
        evtcount = CS_ggH*BR_HZZ*eff*1000*lumi8TeV
...
for ichannel, channel in enumerate(channels):
    ...
    for thismass in masses_8TeV_4:
        ...
        BR_HZZ = myCSW.HiggsBR(15, float(thismass))
        evtcountfile = plot_util.getsumweight_ntuple([thisfile])
        evtcount = 1*CS_ggH*BR_HZZ*eff*1000*lumi8TeV
        print "CS_ggH ({0}) * BR_HZZ ({1}) * eff({2}) * (1000) * lumi7TeV({4}) = {5}".format(CS_ggH, BR_HZZ, eff, Z2lBR, lumi8TeV, evtcount)
...
for ichannel, channel in enumerate(channels):
    # signal gg -> H -> ZZ -> 4l
    for thismass in masses_8TeV_3:
        print ""
        name = "ggH_{0}_{1}_{2}".format(thismass,channel,era)
        print name

        dataset = 'GluGluToHToZZTo4L_M-{0}_{1}-powheg15-pythia6'.format(thismass, era)
        thisfile = '{0}histos{2}_paper/output_{1}_bnn.root'.format(dirin, dataset, channel)
        eff = plot_util.getefficiency(thisfile, "sig_input_Summer12_paper.txt", dataset)
        CS_ggH = myCSW.HiggsCS(1, float(thismass), sqrts)
        BR_HZZ = myCSW.HiggsBR(15, float(thismass))
...
#        print "New weight: " + str(CS_ggH*myCSW.HiggsBR(13, float(thismass))*eff*1000.*lumi8TeV)
        ...
        evtcount = CS_ggH*BR_HZZ #*1000*lumi8TeV
    #    print "CS_ggH ({0}) * BR_HZZ_4l ({1}) * (1000) * lumi7TeV({3}) = {4}".format(CS_ggH, BR_HZZ, lumi8TeV, evtcount)
